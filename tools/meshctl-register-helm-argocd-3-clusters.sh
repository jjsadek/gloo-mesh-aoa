...
# create gloo-mesh ns in cluster1, cluster2, and cluster3
kubectl create ns gloo-mesh --context ${cluster1_context}
kubectl create ns gloo-mesh --context ${cluster2_context}
kubectl create ns gloo-mesh --context ${cluster3_context}
...
kubectl create secret generic relay-root-tls-secret -n gloo-mesh --context ${cluster3_context} --from-file ca.crt=ca.crt
rm ca.crt
...
kubectl create secret generic relay-identity-token-secret -n gloo-mesh --context ${cluster3_context} --from-file token=token
rm token
...
kind: Application
metadata:
  name: gm-enterprise-agent-cluster3
  namespace: argocd
spec:
  destination:
    server: https://kubernetes.default.svc
